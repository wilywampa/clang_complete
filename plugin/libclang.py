...
def _tempfile(content):
  ...
  with open(vim.eval('tempname()'), 'w') as f:
    f.write(content)
  try:
    yield f
  finally:
...
def _escape(s):
  return '"%s"' % decode(s).replace('\\', '\\\\').replace('"', r'\"')

def jumpToLocation(filename, line, column, preview, name):
  with _tempfile('{}\t{}\t{}'.format(name, filename,
      'call cursor({}, {})'.format(line, column))) as f:
    ...
    try:
      ...
      vim.command('let &tags = %s' % _escape(f.name))
      vim.command('tjump %s' % name)
    finally:
      vim.command('let &tags = %s' % _escape(old_tags))
      vim.command('let &wildignore = %s' % _escape(old_wildignore))
    ...
    for d in defs:
      if d is not None and loc != d.location:
        ...
        if loc.file is not None:
          found = True
          jumpToLocation(decode(loc.file.name), loc.line, loc.column, preview,
                         decode(d.spelling or d.displayname))
        break
